...
export const CreateEditCommentModal: FC<Props> = ({ close, refetch, id, type, isEdit, editContent }) => {
  ...
  const handleSubmit = async () => {
    if (type === "reply") {
      await updateQuestion.mutateAsync({ answerContent: content, id: id, gated: undefined });
    } else {
      if (isEdit) {
        await editComment.mutateAsync({ commentId: id, comment: content });
      } else {
        await postComment.mutateAsync({ questionId: id, comment: content, gated: currentUser!.gated });
      }
    }
