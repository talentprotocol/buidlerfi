...
export default function EditProfilePage() {
  ...
  const [gated, setGated] = useState<boolean>(profile.user?.gated || true);
  ...
  const handleSaveProfile = async () => {
    setIsLoading(true);
    await updateUser
      .mutateAsync({ tags: selectedTags, bio: bio, gated })
      .then(async () => {
  ...
  return (
    <Flex y grow component="main" gap2 p={2}>
      ...
      <Flex y gap1>
        <Flex x gap2>
          <Switch checked={gated} onChange={e => setGated(e.target.checked)} />
